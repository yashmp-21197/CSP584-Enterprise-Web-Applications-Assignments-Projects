...
public class Home extends HttpServlet {
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(utility.isLoggedin() && utility.usertype().equalsIgnoreCase("store manager")){
			...
			if(type==null && manufacturer==null && product_id==null){
			...
			if(product_map == null){
				...
				if(product_id != null && !product_id.equalsIgnoreCase("")){
					Product p = product_map.get(product_id);
					ArrayList<Product> list = sorted_product_map.getOrDefault(p.getType(), new ArrayList());
					list.add(p);
					sorted_product_map.put(p.getType(), list);
				}
