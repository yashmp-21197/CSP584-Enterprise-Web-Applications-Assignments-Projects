		...
		if(action.equalsIgnoreCase("order")){
			...
			if(action1 == null){
					...
					if(order_id != null && !order_id.isEmpty()){
						...
						if(orders.containsKey(oid)){
							...
							for (OrderItem oi : orders.get(oid)) 
							{
								...
								pw.print("<td><input type='number' min='1' max='10' name='productquantity' value='"+oi.productquantity+"'></td>");
				...
				if(orders!=null && products!=null){
					...
					{
						int orderid = Integer.parseInt(oid);
						int new_productquantity = Integer.parseInt(pquantity);
						...
						for(OrderItem oi : orders.get(orderid)) 
							{
									...
									{
										Product p = products.get(productid);
										int old_productquantity = oi.productquantity;
										int diff_productquantity = new_productquantity - old_productquantity;
											try{
												String shipdate = oi.shipdate;
												Date cDate = Calendar.getInstance().getTime();
												Date sDate = sdf.parse(shipdate);
												...
												if(days_diff >= 0){
													if(p.getCount() >= diff_productquantity){
														p.setCount(p.getCount() - diff_productquantity);
														boolean ret = MySqlDataStoreUtilities.updateProduct(p);
														OrderItem uoi = new OrderItem(oi.userid, oi.username, oi.useroccupation, oi.userage, oi.useraddress, oi.creditcardno, oi.orderid, oi.purchasedate, oi.shipdate, oi.actualshipdate, oi.productid, oi.productname, oi.producttype, oi.productmanufacturer, new_productquantity, oi.productprice, oi.shippingcost, oi.productdiscount, new_productquantity*(oi.shippingcost + (oi.productprice*(1.0-(oi.productdiscount/100.0)))), oi.reviewrating, oi.trackingid, oi.storeid, oi.storeaddress, oi.zipcode, oi.transactionstatus, oi.iscancelled, oi.isreturned, oi.isdeliveredontime);
														ret = ret & MySqlDataStoreUtilities.updateOrderItem(uoi);
														if(ret==true)
															pw.print("<h4 style='color:red'>Order is Updated.</h4>");
														else
															pw.print("<h4 style='color:red'>Order Update Error.</h4>");
													}else{
														pw.print("<h4 style='color:red'>Store has only "+p.getCount()+" counts of "+p.getName()+" in inventory.</h4>");
													}
												}else{
